...
Run sandbox tools in PPIO while OpenAI runs the agent and maintains session
...
| --- | --- | --- |
| `PPIO_API_KEY` | PPIO console | template build, sandbox lifecycle |
...
session — the first needs only `PPIO_API_KEY`.
...
ppio sbx create openai-agents-api-executor --long-running --timeout 720h
```
...
5. Checking the workspace for the written file
   file contents: ppio executor ok
...
Verify the image before using it in a session. This needs only `PPIO_API_KEY`:
...
| Symptom | Check |
| --- | --- |
| Session stays in `environment.pending` | Executor process alive? If you restrict egress, allowlist `api.openai.com` and `codex-cloud-environments.chatgpt.com`. PPIO allows outbound traffic by default, so no configuration is needed otherwise. |
| Executor dies around 61 seconds | `commands.run` started without `background: true`. |
| Every SDK call fails with 503 and an HTML body | `PPIO_DOMAIN` set to a region the SDK does not recognize. Leave it unset. |
...
| `ppio-sandbox-cli sandbox exec` hangs with no output | Non-interactive stdin is treated as piped input and waited on. Add `</dev/null` to every `exec` in a script. |
...
| `src/verify.ts` | Ten checks against the built image; needs only `PPIO_API_KEY` |
...
- [PPIO Sandbox documentation](https://ppio.com/docs/sandbox/overview)
